...
let detailsLocked = false; // track lock state

if (detailsEl) {
  detailsEl.style.overflowY = 'auto';
  detailsEl.style.maxHeight = '80vh';
  detailsEl.style.paddingRight = '8px';
  detailsEl.style.scrollBehavior = 'smooth';

  container.querySelectorAll('.annotation-text').forEach(el => {
    el.addEventListener('mouseenter', () => {
      if (detailsLocked) return; // don't update while locked
      updateDetailsPanel(el);
    });
    ...
    el.addEventListener('click', e => {
      ...
      if (detailsLocked) {
        ...
        detailsEl.innerHTML = 'Hover over colored text to see annotation details';
      }
    });
  ...
  // If click anywhere else, unlock the panel
  document.addEventListener('click', e => {
    if (detailsLocked && !detailsEl.contains(e.target)) {
      detailsLocked = false;
      detailsEl.classList.remove('locked');
      detailsEl.innerHTML = 'Hover over colored text to see annotation details';
    }
...
function updateDetailsPanel(el) {
  let html = el.dataset.details || 'No labels';

  try {
    const wordText = el.textContent.trim();
    const relatedSets = [];

    // helper to shorten long labelled text (first 5 + last 3 words)
    const shorten = text => {
      const words = text.trim().split(/\s+/);
      if (words.length <= 8) return `“${text}”`;
      const short = [...words.slice(0, 5), '…', ...words.slice(-3)].join(' ');
      return `“${short}”`;
    };
    ...
    (task.annotations || []).forEach(annObj => {
      ...
      (annObj.result || []).forEach(r => {
        if (r.type === 'labels' && r.value?.labels) {
          nodes.push({
            id: r.id,
            text: r.value.text || '',
            startPos: Number(r.value.globalOffsets?.start) || 0,
            endPos: Number(r.value.globalOffsets?.end) || 0
          });
      ...
      const sortedComps = comps.map(c =>
        c.map(id => nodes.find(n => n.id === id)).sort((a, b) => a.startPos - b.startPos)
      );

      sortedComps.forEach(group => {
        if (group.some(n => n.text && n.text.includes(wordText))) {
          const parts = group.map(n => {
            const shortened = shorten(n.text);
            const escaped = shortened.replace(/</g, "&lt;").replace(/>/g, "&gt;");
            // highlight hovered word
            if (normalizeText(n.text).includes(wordText)) {
              return `<span class="relation-word highlight-word">${escaped}</span>`;
            } else {
              return `<span class="relation-word">${escaped}</span>`;
            }
          });
          const connected = parts.join(' | ');
          relatedSets.push(`${userEmail}: ${connected}`);
        }
      });
    });

    if (relatedSets.length > 0) {
      html += `<hr><b>Connected words (by relation):</b><br>${relatedSets
        .map(line => `<div>${line}</div>`)
        .join('')}`;
    }
  } catch (e) {
    console.warn('Relation hover build failed:', e);
  }
...
style.textContent = `
  ...
  .highlight-word {
    background-color: rgba(153, 102, 255, 0.25);
  ...
  #details.locked {
    ...
    border-left: 3px solid rgba(102, 51, 255, 0.4);
  }
